...
*Working Title — v0.1 Draft — May 2026*

> This document captures the full vision, early architectural decisions, user stories, and open questions from the initial ideation session for Strata. It is intended as a living reference to guide development when active building begins. No code has been written yet. This is a thinking document.
...
Music theory scholarship currently has no good tool for building structured, queryable corpora of interpretive formal analysis. The existing landscape:
...
### 2.2 Two Separate Products

> **Critical architectural distinction:** the Analysis Tool and the Library are two separate products. Conflating them from the start will prevent v1 from shipping.

The **Analysis Tool** is a single-file editor. It runs entirely client-side in a browser. No accounts, no server, no database. You open a YouTube URL, build your layers, export a `.strata` file. This is the core tool.

The **Library** is where analyses are shared, browsed, voted on, and commented on. It requires user accounts, a backend, storage, and moderation. It is a separate product that consumes analysis documents produced by the tool.

The file format being open means the Library can exist independently — people can self-host their own. Build the Analysis Tool first. Get it to a state where it is useful for your own scholarship. Then build the Library.
...
- `data` — array of spans or widget-specific data objects
...
In addition to spans, the timeline supports point markers — single timestamps with a label. "This is where the anthem enters." "Key change here." These are not sections, just flags. They need to be in the data model from v1 even if the UI for them is simple, because bolting them on later is awkward.
...
### 5.5 Widget Developer / Community Contributor
...
### 6.4 v3+: Timestamped Written Analysis
...
This widget can import span data from the form diagram layer — surfacing the right text block based on span ID reference. This is the first instance of inter-widget data sharing.
...
## 7. Corpus Analysis & The Library

### 7.1 Corpus Analysis Philosophy
...
### 7.2 What Makes Corpus Analysis Possible
...
### 7.3 The Library (Future Product)

The Library is where analyses are shared publicly, browsed, rated, and commented on. It is a separate product from the Analysis Tool and should not be built until the Analysis Tool is stable and in active use.

The Library is also an analytical database, not just a sharing platform. When analyses are uploaded, their span data is indexed. Corpus queries become possible across the community's collective work — not just a single scholar's local files.

Community voting and comment features allow quality to surface organically without requiring a formal editorial process.
...
- How does the Library work technically? (Database choice, hosting, authentication) — Deferred until Analysis Tool is stable.
...
- Should there be a desktop application version (Electron/Tauri) for analysts who work with local audio files predominantly? — Nice to have, not v1.
- Is there a native Anthemics integration? — Deferred. Keep tools separate until both are stable. The file format being open means integration is always possible later.
...
*Project Vision & Architectural Guiding Document v0.1*
*Generated May 2026 — Update this document as decisions are made.*
